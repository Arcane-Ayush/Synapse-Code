      ...
      <SEO
        ...
        image={
          import.meta.env.VITE_SITE_URL
            ? `${import.meta.env.VITE_SITE_URL}/og-activities.png`
            : undefined
        }
        url={
          import.meta.env.VITE_SITE_URL
            ? `${import.meta.env.VITE_SITE_URL}/activities`
            : undefined
        }
        jsonLd={{
      ...
      <div className="text-center mb-16">
        <h2
          className={`text-5xl md:text-6xl font-black mb-4 ${
            theme === themes.ANIME
              ? "text-slate-800 drop-shadow-sm font-serif italic"
              : theme === themes.ARCADE
                ? "text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-yellow-500 uppercase tracking-tighter"
        ...
        <p
          className={`max-w-2xl mx-auto text-lg ${
            theme === themes.ANIME
              ? "text-slate-500 font-serif italic"
              : theme === themes.ARCADE
                ? "text-pink-400 font-mono"
                : "text-cyan-400/60"
          }`}
        >
          {theme === themes.ANIME
        ...
        {loading && (
          <div className="grid md:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="p-6 rounded-2xl border border-white/10 bg-white/5 animate-pulse h-40"
              />
            ))}
          </div>
        )}
