...
export function Home() {
  ...
  return (
    <div className="relative overflow-hidden w-full">
      <SEO
        ...
        image={
          import.meta.env.VITE_SITE_URL
            ? `${import.meta.env.VITE_SITE_URL}/og-home.png`
            : undefined
        }
        url={
          import.meta.env.VITE_SITE_URL
        ...
        jsonLd={{
          ...
          logo: import.meta.env.VITE_SITE_URL
            ? `${import.meta.env.VITE_SITE_URL}/logo.png`
            : "",
          sameAs: ["https://github.com/Arcane-Ayush"],
        }}
      />
          ...
          >
            ...
            >
              ...
              <span className="text-sm font-medium text-muted-foreground">
                Ignite the Flare: Peer-to-Peer Learning
              </span>
            </Motion.div>
            ...
            <Motion.p
              variants={itemVariants}
              className="text-xl md:text-2xl text-muted-foreground max-w-xl mx-auto md:mx-0 font-light leading-relaxed"
            >
              Google Club CU is a peer to peer student community for aspiring
      ...
      <section
        className={`py-24 relative ${theme === themes.ANIME ? "bg-white/30" : "bg-black/20"}`}
      >
...
function FeatureCard({ icon, title, desc, theme }) {
  ...
  return (
    <Motion.div
      whileHover={{ y: -5 }}
      className={`p-8 rounded-2xl backdrop-blur-lg transition-colors border ${
        isAnime
    ...
    >
      <div
        className={`mb-4 p-3 rounded-xl inline-block ${
          isAnime ? "bg-slate-50/50" : "bg-white/5"
        }`}
      >
        {icon}
      </div>
      <h3
        className={`text-2xl font-bold mb-3 ${isAnime ? "text-slate-800" : "text-white"}`}
      >
        {title}
      </h3>
      <p
        className={`leading-relaxed ${isAnime ? "text-slate-600 font-medium" : "text-muted-foreground"}`}
      >
        {desc}
      </p>
    </Motion.div>
