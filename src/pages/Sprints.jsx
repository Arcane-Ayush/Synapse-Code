      ...
      <SEO
        ...
        image={
          import.meta.env.VITE_SITE_URL
            ? `${import.meta.env.VITE_SITE_URL}/og-sprints.png`
            : undefined
        }
        url={
          import.meta.env.VITE_SITE_URL
            ? `${import.meta.env.VITE_SITE_URL}/sprints`
            : undefined
        }
      />
      {loading && (
      ...
      {theme === themes.ARCADE ? (
          ...
          <div className="max-w-4xl mx-auto border-4 border-double border-purple-500/50 p-6 bg-black/90 relative shadow-[0_0_30px_rgba(168,85,247,0.2)]">
            ...
            <div className="space-y-3 font-mono mt-2">
              {sortedLeaderboard.map((team, index) => (
                <div
                  key={team.name}
                  className="flex items-center justify-between border-b border-purple-900/30 pb-2 hover:bg-purple-900/20 transition px-2 rounded"
                >
                  <div className="flex items-center gap-4">
                    <span
                      className={`text-xl font-bold ${index === 0 ? "text-yellow-400 drop-shadow-md" : "text-gray-500"}`}
                    >
                      #{team.rank}
                    </span>
                  ...
                  <span className="text-purple-400 font-bold tracking-wider">
                    {team.points}{" "}
                    <span className="text-xs opacity-50 text-white">XP</span>
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-16">
          ...
          <div className="text-center">
            ...
            <h2
              className="text-5xl md:text-6xl font-black text-slate-800 drop-shadow-sm mb-2"
