...
export function ThemeEffects() {
    ...
    const isTouchNoHover = typeof window !== 'undefined' && window.matchMedia('(hover: none)').matches;
    ...
    useEffect(() => {
        ...
        const step = () => {
            ...
            if (tick % spawnEvery === 0) {
                ...
                const p = { x, y, vx: 0, vy: -0.4, life: 1, size: theme === themes.ARCADE ? 8 : theme === themes.ANIME ? 6 : 3 };
            ...
            for (let i = 0; i < pointsRef.current.length; i++) {
                ...
                if (theme === themes.ANIME) {
                    ...
                    ctx.beginPath();
                    ctx.arc(p.x + (Math.random() - 0.5) * 6, p.y + (Math.random() - 0.5) * 6, p.size / 2, 0, Math.PI * 2);
                    ctx.fill();
                }
            }
    ...
    return (
        <div className="pointer-events-none fixed inset-0 z-50">
            {!prefersReduced && !isTouchNoHover && <canvas ref={canvasRef} className="w-full h-full" aria-hidden="true" />}
        </div>
    );
}
