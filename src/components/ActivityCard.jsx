  ...
  return (
    <Link to={`/activities/${activity.id}`} className="block">
      <Motion.div
        ...
        className="flex flex-col md:flex-row gap-4 p-4 border border-border rounded-lg bg-card hover:bg-accent/50 transition-colors cursor-pointer"
      >
        ...
        <div className="flex-grow">
          <div className="flex justify-between items-start">
            ...
            <span
              className={`text-xs px-2 py-1 rounded-full border ${isUpcoming ? "bg-green-100 text-green-700 border-green-200" : "bg-gray-100 text-gray-700 border-gray-200"}`}
            >
              {activity.status}
            </span>
          </div>
