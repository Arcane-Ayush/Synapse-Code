          ...
          <div className="mt-4">
            <div className="flex flex-wrap gap-1.5 mb-4">
              {project.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 text-[10px] font-medium bg-secondary/50 border border-secondary text-secondary-foreground rounded-md"
                >
                  {tag}
                </span>
              ))}
            ...
            <Link
              ...
              className={`block w-full py-2.5 rounded-xl font-bold text-sm transition-all active:scale-[0.98] text-center shadow-sm ${
                theme === themes.ANIME
      ...
      <div className="flex flex-col flex-1 py-1">
        ...
        <div
          className={cn(
            ...
            theme === themes.BASIC ? "text-gray-400" : "text-muted-foreground",
          )}
        >
        ...
        <div className="flex flex-wrap gap-2 mt-auto">
          {project.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 text-[10px] font-medium bg-muted text-muted-foreground rounded-full border border-border/50"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="mt-3">
          <Link
            to={`/projects/${project.id}`}
            className={cn(
              "inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-bold transition-all active:scale-[0.98] border",
              theme === themes.ANIME
                ? "bg-slate-800 text-white border-white/20 hover:bg-pink-500"
                : theme === themes.ARCADE
                  ? "bg-black text-[#df00ff] border-[#df00ff]/40 hover:bg-[#df00ff]/10"
