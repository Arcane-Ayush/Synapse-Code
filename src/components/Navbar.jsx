    ...
    switch (theme) {
      case themes.ARCADE: // Arcade: Neon, Retro, Boxy
        return {
          ...
          dockStyle: {
            backgroundImage:
              "linear-gradient(#000, #000), linear-gradient(90deg, #00ff00, #ff00ff)",
          ...
          itemActive:
            "bg-[#df00ff] text-black font-bold shadow-[0_0_10px_#df00ff]", // Neon Purple Active
          itemInactive:
            "text-[#df00ff]/70 hover:text-[#df00ff] hover:bg-[#df00ff]/10", // Neon Purple Text
          ...
          mobileMenu:
            "bg-black border-2 border-[#df00ff] rounded-none font-mono text-[#df00ff]",
        };
      ...
      default: // Anime (Sakura): The original Glassmorphism
        return {
          ...
          iconContainer:
            "bg-primary/20 text-primary group-hover:rotate-12 transition-transform",
    ...
    >
      <div className="w-full max-w-7xl flex items-center justify-between pointer-events-auto">
        ...
        <Link
          to="/"
          className={cn(
            "group flex items-center gap-2 px-5 py-2.5 transition-all duration-300 rounded-full",
            styles.logo,
          )}
        >
        ...
        {/* 3. Right: Auth + Mobile Toggle */}
        <div className="flex items-center gap-3">
          {user ? (
            <Link
              to="/dashboard"
              className="flex items-center gap-2 px-3 py-2 rounded-full bg-white/10 border border-white/10"
              title="Dashboard"
            >
              <img
            ...
            <button
              ...
              className={cn("px-3 py-2 rounded-full text-white transition-colors", styles.logo)}
            >
              Sign In
            </button>
          )}
      ...
      <AnimatePresence>
        {isOpen && (
          <Motion.div
            ...
            className={cn(
              "absolute top-24 left-4 right-4 p-4 rounded-3xl shadow-2xl md:hidden pointer-events-auto",
              styles.mobileMenu,
            )}
          >
              ...
              {navItems.map((item) => (
                <Link
                  ...
                  className={cn(
                    ...
                    theme === themes.ARCADE &&
                      location.pathname === item.path &&
                      "bg-[#df00ff] text-black rounded-none",
                    theme === themes.ARCADE &&
                      location.pathname !== item.path &&
                      "text-[#df00ff] hover:text-[#df00ff] hover:bg-[#df00ff]/10 rounded-none",
                  )}
                >
                  {item.name}
                </Link>
