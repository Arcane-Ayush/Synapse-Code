        ...
        {currentSprint.tasks.map((task, i) => (
          ...
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <span className="font-black text-2xl text-slate-200 group-hover:text-pink-200 transition-colors">
                #{task.id}
              </span>
            </div>
            <div className="text-sm text-slate-500 mb-6 font-medium">
              {task.assignedTo === "All" ? (
                <span className="font-black text-white bg-gradient-to-r from-purple-500 to-indigo-500 px-3 py-1 rounded-full shadow-lg animate-pulse">
                  🌍 WORLD EVENT
                </span>
              ) : Array.isArray(task.assignedTo) ? (
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-xs font-bold text-pink-400 uppercase tracking-widest border border-pink-200 px-1.5 rounded">
                    Co-op
                  </span>
                  {task.assignedTo.map((team) => (
                    <span
                      key={team}
                      className="font-bold text-pink-500 bg-pink-50 px-2 py-0.5 rounded-md border border-pink-100 shadow-sm text-xs"
                    >
                      {team}
                    </span>
                  ))}
                ...
                <span className="font-bold text-pink-500 bg-pink-50 px-3 py-1 rounded-full border border-pink-100 shadow-sm animate-pulse">
                  ✨ Available Quest ✨
                </span>
              )}
            </div>
