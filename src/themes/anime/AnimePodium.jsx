    ...
    <div className="w-full max-w-5xl mx-auto">
        ...
        {top3.map((team, index) => {
          ...
          let height = isFirst
            ...
            : isSecond
              ...
              : "h-40 md:h-56";
          let color = isFirst
            ? "bg-gradient-to-b from-yellow-300 via-yellow-400 to-orange-400 border-yellow-200"
            : isSecond
              ? "bg-gradient-to-b from-slate-200 via-slate-300 to-slate-400 border-slate-200"
              : "bg-gradient-to-b from-orange-300 via-orange-400 to-red-400 border-orange-200";
          let glow = isFirst
            ? "shadow-[0_20px_50px_rgba(250,204,21,0.4)]"
            : "shadow-xl";
          let delay = isFirst ? 0.4 : isSecond ? 0.2 : 0.6;
          let rankColor = isFirst
            ? "text-yellow-600"
            : isSecond
              ? "text-slate-600"
              : "text-orange-700";
          ...
          return (
            ...
            >
              ...
              >
                <div className="relative">
                  ...
                  <img
                    src={team.avatar}
                    alt={team.name}
                    className="relative w-20 h-20 md:w-28 md:h-28 rounded-full border-4 border-white shadow-lg object-cover bg-white"
                  />
                  <div
                    className={`absolute -bottom-3 -right-2 w-10 h-10 rounded-full bg-white flex items-center justify-center font-black text-xl shadow-lg ${rankColor}`}
                  >
                    {team.rank}
                  </div>
                </div>
              ...
              <div className="mb-8 text-center bg-white/20 backdrop-blur-sm rounded-xl px-4 py-2 mx-4 w-3/4">
                <span className="block text-2xl md:text-4xl font-black text-white drop-shadow-md">
                  {team.points}
                </span>
      ...
      <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/50 mb-16">
        <h4 className="text-xl font-bold mb-6 text-slate-700 flex items-center gap-2">
          <span className="bg-slate-100 p-2 rounded-lg">🎖️</span> Honorable
          Mentions
        </h4>
        <div className="flex gap-6 overflow-x-auto pb-4 px-2 no-scrollbar">
          {others.map((team, i) => (
            ...
            >
              <span className="font-black text-3xl text-slate-200 group-hover:text-pink-400 transition-colors">
                #{team.rank}
              </span>
              <img
                src={team.avatar}
