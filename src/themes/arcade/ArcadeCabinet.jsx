...
export function ArcadeCabinet() {
  ...
  const cabinetMat = (
    <meshStandardMaterial color="#1a0b2e" roughness={0.3} metalness={0.8} />
  );
  const trimMat = (
    ...
    <group scale={[0.8, 0.8, 0.8]}>
      <group ref={group}>
        ...
        <group position={[0, 0.6, -0.2]}>
          {/* Back Spine */}
          <Box args={[1.2, 1.2, 0.8]} position={[0, 0, -0.2]}>
            {cabinetMat}
          </Box>
          {/* Top Overhang (Marquee) */}
          <Box args={[1.3, 0.3, 1.4]} position={[0, 0.75, 0.1]}>
            {trimMat}
          </Box>
          {/* Marquee Text Area */}
        ...
        <Box args={[0.1, 2.6, 1.2]} position={[-0.65, 0.1, 0]}>
          {trimMat}
        </Box>
        <Box args={[0.1, 2.6, 1.2]} position={[0.65, 0.1, 0]}>
          {trimMat}
      ...
      {/* Orbiting Pixels */}
      <group ref={particlesRef}>{pixels}</group>
    </group>
  );
}
