        ...
        {projects.map((project, index) => {
          ...
          return (
            ...
            >
              ...
              >
                {isActive && (
                  <div className="absolute inset-0 rounded-full animate-ping bg-white opacity-50" />
                )}
              </Motion.div>
                ...
                >
                  ...
                  <div className="relative h-32 w-full overflow-hidden">
                    ...
                    <div className="absolute bottom-2 left-4">
                      <h3
                        className={`font-mono font-bold text-lg ${isActive ? "text-cyan-300" : "text-gray-200"}`}
                      >
                        {project.title}
                      </h3>
                    </div>
                  ...
                  <div className="px-4 py-3 space-y-2">
                    <p
                      className={`text-xs line-clamp-3 font-mono ${isActive ? "text-gray-300" : "text-gray-500"}`}
                    >
                      {project.description}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1 pt-1">
                      {project.tags.slice(0, 3).map((tag) => (
                        <span
                          key={tag}
                          className={`text-[9px] px-1.5 py-0.5 rounded border ${isActive ? "border-cyan-500/30 text-cyan-200" : "border-white/10 text-gray-600"}`}
                        >
                          {tag}
                        </span>
                      ))}
                    ...
                    <div
                      className={`flex gap-3 pt-2 transition-opacity duration-300 ${isActive ? "opacity-100" : "opacity-40"}`}
                    >
                      {project.githubUrl && (
                        <a
                          ...
                          className="text-cyan-400 hover:text-white text-xs flex items-center font-mono font-bold"
                        >
