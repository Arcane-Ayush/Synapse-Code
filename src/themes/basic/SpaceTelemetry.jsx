  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 font-mono text-cyan-500">
      ...
      <div className="border border-cyan-900/50 rounded-md bg-black/60 backdrop-blur-sm p-6 relative overflow-hidden">
        <div className="absolute top-0 right-0 p-2 text-[10px] text-cyan-700 border-l border-b border-cyan-900/50">
          SYSTEM_MONITOR_V2
        </div>
      ...
      <div className="border border-cyan-900/50 rounded-md bg-black/60 backdrop-blur-sm p-6 relative">
        <div className="flex justify-between items-start mb-6 border-b border-cyan-900/50 pb-4">
          <div>
            <div className="text-[10px] text-cyan-600 tracking-widest mb-1">
              MISSION_OBJECTIVE
            </div>
          ...
          <div className="text-right">
            ...
            <span className="text-cyan-300 font-bold border border-cyan-500/30 px-2 py-1 bg-cyan-950/30">
              {currentSprint.deadline}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {currentSprint.tasks.map((task) => (
            <div
              key={task.id}
              className="bg-cyan-950/10 border border-cyan-900/30 p-3 flex items-center justify-between group hover:border-cyan-500/50 transition-colors"
            >
                  ...
                  <div className="text-xs text-cyan-400 mt-1 uppercase tracking-wider font-semibold">
                    {task.assignedTo === "All" ? (
                      ...
                      <span className="text-orange-400 font-bold">
                        MULTIPLE SIGNALS: {task.assignedTo.join(" + ")}
                      </span>
                    ) : task.assignedTo ? (
                      `LOCKED ON: ${task.assignedTo}`
                    ) : (
                      <span className="text-cyan-400 animate-pulse opacity-70">
                        &gt;&gt; UNCLAIMED SIGNAL &lt;&lt;
                      </span>
                    )}
                  </div>
